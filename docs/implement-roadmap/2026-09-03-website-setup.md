...
**技术栈：**Node.js 24.20.0、pnpm 11.25.0、Astro 7.3.0、TypeScript 6.0.3、MDX、Shiki、KaTeX、Pagefind、Lucide Astro、Vitest、Playwright、axe-core、GitHub Pages Actions。
...
| `lucide-astro` | `0.556.0` | dependency |
...
- [ ] **步骤 7：验证初始化结果**
  ...
  预期：Astro 为 `7.3.0`，检查和构建通过，`dist/` 包含 Pagefind 输出。
...
- [ ] **步骤 6：实现终端路径和工具栏**
  ...
  路径逐级链接，当前段不可点击，`cd ..` 单独作为第二行父链接。工具栏顺序固定为 `中 / EN`、主题、搜索；使用 `lucide-astro` 和无障碍名称。
